...
class monte_carlo_sims():
    '''
        ...
        Inputs:-
        circuit = Provide the circuit object on which Monte carlo simulations
        need to be performed
        no_of_simulations = Total number of simulations needed to be performed
        tol = tolerence for the elements(Assumed to be equal for all)
    ...
    def create_sims(self, tol):
        for i in range(self.no_of_simulations):
            ...
            tasks['R'] = {}
            tasks['C'] = {}
            model_nmos = self.libraries_nmos[np.random.randint(
                0, len(self.libraries_nmos))]
            model_pmos = self.libraries_pmos[np.random.randint(
                0, len(self.libraries_pmos))]
            for j in self.mosfet_nmos:
                length = str(float(j.length[:-1]) +
                             (np.random.normal()*tol/100)) + 'u'
                width = str(float(j.width[:-1]) +
                            (np.random.normal()*tol/100)) + 'u'
            ...
            for j in self.resistors:
                tasks['R'][j] = str(float(j.resistance[:-1])
                                    + np.random.normal() * tol / 100) \
                    + 'k'
            for j in self.capacitors:
                tasks['C'][j] = str(float(j.resistance[:-1])
                                    + np.random.normal() * tol / 100) \
                    + 'u'
    ...
    def solve(self):
        ...
        leftover_threads = self.no_of_simulations - \
            (threads_per_core*no_of_cpu)

        if (self.no_of_simulations < 12):
            a = [pool.apply_async(child, args=(i, 1))
                 for i in range(self.no_of_simulations)]
            pool.close()
            pool.join()
        else:
...
if __name__ == "__main__":
    ...
    p.add_argument('--sim', type=str, default='op', choices=['op', 'mc', 'pc'],
                   help='Which type of simulation to be performed')
    p.add_argument('-o', '--output', type=str, default="None",
                   help='File name (JSON format)')
    p.add_argument('-n', '--nsims', type=int, default=100,
                   help="Enter number of simulations for Monte Carlo")
    p.add_argument('-t', '--tol', type=float, default=10,
                   help="Enter tolerence for each element")
    p.add_argument('--param', type=str, default="",
                   help="Enter w, l of each mosfet")

    args = p.parse_args()
    dims = [str(i) for i in args.param.split(',')]
    if (len(dims) != 1):
        for i in range(0, len(dims), 3):
            if (dims[i+1] != '0'):
                circuit[dims[i]].length = dims[i+1]+'u'
            if (dims[i+2] != '0'):
                circuit[dims[i]].width = dims[i+2]+'u'
    if (args.sim == 'pc'):
        obj = process_corner_sims(circuit, list(
            spice_library.models), file_name='result.json')
    elif (args.sim == 'op'):
        obj = op(circuit=circuit, file_name=args.output)
    elif args.sim == 'mc':
        obj = monte_carlo_sims(circuit=circuit, file_name=args.output,
                               spice_library=list(spice_library.models),
                               no_of_simulations=args.nsims,
                               tol=args.tol)
    print(circuit)
